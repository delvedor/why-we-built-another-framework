...
window.addEventListener('load', function () {
  ...
  if (!document.body) {
    throw new Error(
      'diapo could not find a body element on this page, so it is exiting'
    )
  }
  ...
  if (!slideDivs.length) {
    throw new Error(
      "diapo couldn't find any slides in this presentation: " +
  ...
  if (diapo.audio) {
    diapo.playControl = body.appendChild(ce('div'))
    diapo.playControl.style.cssText = 'padding:5px;color:#aaa;'
    diapo.playControl.addEventListener('click', onClickPlay)
  ...
  function goToAudio (n) {
    if (!diapo.audio || !diapo.playControl) return
    diapo.playControl.style.cssText =
      n === 0 ? 'display:none' : 'padding:5px;color:#aaa;'
    if (n === 0) {
      diapo.audio.pause()
    } else {
      diapo.audio.currentTime = diapo.audio.textTracks[0].cues[n - 1].startTime
      if (diapo.audio.paused) diapo.audio.play()
    }
  }
  ...
  function go (n, dontSeek, force) {
    ...
    n = Math.max(0, Math.min(diapo.length - 1, n))
    // Avoid doing extra work if we're going from a slide to itself.
    if (!force && diapo.current === n) return
    diapo.current = n
    ...
    if (slideDiv.hasAttribute('data-time-to-next')) {
      ...
      if (isNaN(timeToNext)) {
        throw new Error(
          'diapo encountered a bad value for the time-to-next string: ' +
            timeToNextStr
        )
      }
  ...
  function onClickPlay (e) {
    if (diapo.audio.paused) {
      diapo.current === 0 ? forward() : diapo.audio.play()
    } else diapo.audio.pause()
    e.stopPropagation()
  }

  function onAudioUpdate () {
    if (!diapo.playControl) return
    diapo.playControl.innerHTML = diapo.audio.paused ? '&#9654;' : '&#9208;'
    if (!diapo.audio.paused) {
      return
    }
    for (var ci = 0; ci < diapo.audio.textTracks[0].cues.length; ci++) {
      if (
        diapo.audio.textTracks[0].cues[ci].startTime <= diapo.audio.currentTime &&
        diapo.audio.textTracks[0].cues[ci].endTime > diapo.audio.currentTime &&
        diapo.current - 1 !== ci
      ) {
        return go(ci + 1, true)
      }
